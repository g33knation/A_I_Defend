      ...
      <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100 lg:col-span-2">
        <div class="flex justify-between items-center mb-4">
          ...
          <div class="flex space-x-2">
            <button class="px-3 py-1 text-sm rounded-full bg-blue-50 text-blue-600">24h</button>
            <button class="px-3 py-1 text-sm rounded-full text-gray-500 hover:bg-gray-50">7d</button>
            <button class="px-3 py-1 text-sm rounded-full text-gray-500 hover:bg-gray-50">30d</button>
          </div>
        </div>
      ...
      <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
        ...
        <div class="space-y-4">
          <div v-for="service in systemStatus" :key="service.name" class="flex items-center justify-between">
            <div class="flex items-center">
              <span :class="statusColor(service.status)" class="w-2.5 h-2.5 rounded-full mr-3"></span>
              <span class="text-sm font-medium text-gray-700">{{ service.name }}</span>
            </div>
            <span class="text-xs px-2 py-1 rounded-full" :class="statusBadgeColor(service.status)">
              {{ service.status }}
            </span>
          </div>
        </div>
        <div class="mt-6 pt-4 border-t border-gray-100">
          ...
          <button @click="runQuickScan" class="mt-4 w-full px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors flex items-center justify-center">
      ...
      <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-100">
          <div class="flex items-center justify-between">
            ...
            <router-link to="/detections" class="text-sm text-blue-600 hover:underline">View All</router-link>
          </div>
        </div>
        <div class="divide-y divide-gray-100">
          <div v-if="recentDetections.length === 0" class="p-6 text-center text-gray-500">
            <p>No recent detections found</p>
          </div>
          <div v-else>
            <div v-for="detection in recentDetections" :key="detection.id" class="p-4 hover:bg-gray-50 transition-colors">
              <div class="flex items-start">
                ...
                <div class="ml-3 flex-1">
                  <div class="flex items-center justify-between">
                    <p class="text-sm font-medium text-gray-900">{{ detection.message || 'Suspicious activity detected' }}</p>
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium" :class="getDetectionBadgeClass(detection.type)">
      ...
      <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-100">
          <div class="flex items-center justify-between">
            ...
            <router-link to="/events" class="text-sm text-blue-600 hover:underline">View All</router-link>
          </div>
        </div>
        <div class="divide-y divide-gray-100">
          <div v-if="recentEvents.length === 0" class="p-6 text-center text-gray-500">
            <p>No recent events found</p>
          </div>
          <div v-else>
            <div v-for="event in recentEvents" :key="event.id" class="p-4 hover:bg-gray-50 transition-colors">
              <div class="flex items-start">
                ...
                <div class="ml-3 flex-1">
                  <div class="flex items-center justify-between">
                    <p class="text-sm font-medium text-gray-900">{{ event.source || 'System' }}</p>
                    <span class="text-xs text-gray-500">{{ formatTimeAgo(event.timestamp || event.created_at) }}</span>
                  </div>
                  <p class="mt-1 text-sm text-gray-500">
                    {{ event.type || 'Event' }}
                  </p>
